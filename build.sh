...
case "$COMMAND" in
  ...
  semver-check)
    ...
    OLD_SO="$OLD_DIR/build/liba_paxos_net_library_shared.so"
    NEW_SO="build/liba_paxos_net_library_shared.so"
  ...
  coverage)
    ...
    llvm-cov report "$COV_BUILD_DIR/liba_paxos_net_library_debug.a" \
